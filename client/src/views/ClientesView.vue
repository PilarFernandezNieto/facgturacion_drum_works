  ...
  <div class="space-y-6">
    ...
    <div class="flex bg-white p-1 rounded-xl shadow-sm border border-slate-200 w-fit">
      <button 
        ...
        :class="filtroTipo === 'todos' ? 'bg-slate-100 text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'"
      ...
      <button 
        ...
        :class="filtroTipo === 'alumno' ? 'bg-blue-100 text-blue-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'"
        class="px-4 py-2 rounded-lg text-sm font-bold transition"
      >
        Alumnos
      </button>
      <button 
        ...
        :class="filtroTipo === 'bolo' ? 'bg-purple-100 text-purple-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'"
        class="px-4 py-2 rounded-lg text-sm font-bold transition"
      >
        Bolos
      </button>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      <table class="w-full text-left border-collapse">
        ...
        <tbody v-if="!cargando && clientesFiltrados.length">
          <tr v-for="cliente in clientesFiltrados" :key="cliente.id" class="border-b border-slate-50 hover:bg-slate-50/50 transition">
            <td class="px-6 py-4">
              <div class="font-medium text-slate-800">{{ cliente.nombre }}</div>
              <div class="text-xs text-slate-400">NIF: {{ cliente.nif_cif || 'Sin NIF' }}</div>
            </td>
            <td class="px-6 py-4">
              <span 
                :class="cliente.tipo === 'alumno' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'"
            ...
            <td class="px-6 py-4 text-slate-600">
                <div v-if="cliente.tipo === 'alumno'">{{ cliente.curso || '-' }}</div>
            ...
            <td class="px-6 py-4 text-center space-x-2">
              <button @click="abrirModal(cliente)" class="text-blue-500 hover:bg-blue-50 p-2 rounded-lg transition" title="Editar">
                ✎
              </button>
              <button @click="eliminarCliente(cliente.id)" class="text-red-500 hover:bg-red-50 p-2 rounded-lg transition" title="Eliminar">
                🗑
              </button>
            </td>
          </tr>
        ...
        <tbody v-else>
          <tr>
            <td colspan="5" class="px-6 py-12 text-center text-slate-400">
              {{ cargando ? 'Cargando datos...' : 'No hay registros que coincidan con el filtro.' }}
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="mostrarModal" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl animate-in fade-in zoom-in duration-200">
        <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-2xl">
          <h3 class="text-xl font-bold text-slate-800">{{ editando ? 'Editar Cliente' : 'Nuevo Cliente' }}</h3>
        ...
        <form @submit.prevent="guardarCliente" novalidate class="p-6 space-y-4">
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="col-span-1">
              <label class="block text-sm font-medium text-slate-700 mb-1">{{ formulario.tipo === 'alumno' ? 'Cuota Mensual' : 'Precio Base Bolo' }} (€)</label>
            ...
            <div v-if="formulario.tipo === 'alumno'" class="col-span-2 animate-in slide-in-from-top-2">
          ...
          <div class="pt-4 flex gap-3">
            ...
            <button type="submit" class="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-semibold shadow-md shadow-blue-100">Guardar Cliente</button>
          </div>
        </form>
      </div>
    </div>
